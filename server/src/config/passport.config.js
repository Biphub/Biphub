...
export const setupPassport = () => {
  ...
  */
  ...
  (email, password, done) => {
    models.User.findOne({ email: email.toLowerCase() },
    (err, user: UserInstance) => {
      if (err) { return done(err) }
      if (!user) {
        return done(undefined, false, { message: `Email
         ${email} not found.` })
      }
      user.comparePassword(password, (err: Error, isMatch: boolean) => {
        ...
        return done(undefined, false, { message: 'Invalid
         email or password.' })
      })
    })
  }))
    ...
    if (req.user) {
      models.User.findOne({ facebook: profile.id }, (err, existingUser)
       => {
        if (err) { return done(err) }
        if (existingUser) {
          req.flash('errors', { msg: 'There is already a Facebook
          ...
          models.User.findById(req.user.id, (err, user: any) => {
            ...
            user.save((err: Error) => {
              req.flash('info', { msg: 'Facebook account
               has been linked.' })
              done(err, user)
            })
          })
      ...
      => {
        ...
        models.User.findOne({ email: profile._json.email },
